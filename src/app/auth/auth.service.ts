      ...
      .subscribe(
        res => {
          if(res){
            ...
            localStorage.setItem('typeU', JSON.stringify(res.account.role_id));
            // console.log(res);
      ...
      .pipe(
        map(res => {
          ...
          if (res?.user?.token) {
            ...
            localStorage.setItem('typeU', JSON.stringify(res.user?.account?.role_id));
