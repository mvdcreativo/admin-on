...
export class ImagesService {
  ...
  // updateImages(data){
  //   this.http.post<Product>(`${environment.API}courses`, data)
  // }

  getImages() {
    this.http.get<Product>(`${environment.API}courses`)
  ...
  uploadImage(data: Product, files: FileList, index?: number) {
    ...
    return this.http.post(`${environment.API}courses/${data.id}`, formData, {
      observe: 'events',
      reportProgress: true
    }).subscribe(
  ...
  removeImageId(imageID, id) {
    ...
    return this.http.delete<Response>(`${environment.API}courses/${id}`, options).pipe(
      take(1),
      map( v => v.data)
    ).subscribe(
      res => {
